    ...
    public class ProduitsController : Controller
    {
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (produit != null)
            {
                ...
                var lignePanier = _context.LignePanier.FirstOrDefault(p => p.ProduitID == produit.ProduitID);
                var panier = _context.Panier.FirstOrDefault(p => p.PanierID== lignePanier.PanierID);

                panier.Total = panier.Total -( lignePanier.Total * lignePanier.NbreArticle );



                LigneBackup backup = new LigneBackup(lignePanier.LignePanierID, lignePanier.PanierID, lignePanier.ProduitID, lignePanier.NbreArticle, lignePanier.Total, lignePanier?.produit?.Prodname);
                _context.Add(backup);
                await _context.SaveChangesAsync();


                lignePanier.Total =0;
                lignePanier.NbreArticle =0;
                lignePanier.ProduitID =produit.ProduitID;
